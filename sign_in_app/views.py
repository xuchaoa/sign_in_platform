...
def session_required(function):  #判断是否登陆
    def check_session(request,*args,**kwargs):
        ...
        if not session:
            ...
            if not user:
                return JsonResponse({'msg':'no this user in database','code':202})  #用户可能在尝试伪造sessionid，或者session过期，跳转登陆
            else:
...
def log_json(request,*args,**kwargs):
    ...
    if current_username == 'admin':
        log_dic = []
        results = session.query(arp_log).order_by(arp_log.id.desc()).limit(20).all()  # 10条倒序
